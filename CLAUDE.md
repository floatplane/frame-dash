...
- **Timeframe-style sensors** — supports HA template sensors using the `"icon,Label"` CSV format from Timeframe. A `sensor` listed under `watched_entities` is surfaced when its state contains a comma (so an HA template can toggle visibility by outputting the CSV only when a condition holds, else an empty string). Note: entities must be listed in `watched_entities` — there is no auto-discovery — and the e-ink template renders only the label (text after the comma); the icon part is ignored.
- **Low-battery scan** — when `low_battery_enabled`, `HAClient.get_low_battery()` pulls `/api/states` and auto-discovers every `device_class: battery` entity: numeric `%` sensors below `low_battery_threshold` and battery `binary_sensor`s that are `on`. Values outside 0–100 (e.g. a `-1` unavailable placeholder) are skipped, `battery_exclude` substrings filter out noise (phones, watches), and the result is one synthetic Timeframe-style status item. This is the one rule done in-app rather than via an HA template, because battery scanning is generic and tedious to template per-device.
- **Bottom stats strip** — the footer band (`.eink__footer`) is a flex row: ambient stat tiles on the left, the "Updated" time on the right. The vehicle widget (`config.vehicle_*` entities → `VehicleData`) lives here: a name label, a CSS-width-filled battery glyph (`.batt` / `.batt__fill`), a standalone charging bolt to the left of the battery when charging, and the range. The vehicle's battery SOC entity is auto-excluded from the low-battery scan to avoid a duplicate alert. The strip is built to hold more tiles later (solar, etc.).
