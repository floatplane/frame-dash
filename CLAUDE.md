...
- **Attention-only status** — doors/locks/lights/covers are only shown when they're in a "problem" state (unlocked, open, on). The `EntityState.is_problem` property in `ha_client.py` defines this logic per domain. Covers alert when `open` only for closure device classes (garage/door/gate/window), so shades/blinds are ignored; a `garage` cover renders the clean label "Garage door open" rather than its (often ugly) entity name.
